...
import { CheckCircle2, ImagePlus, Plus, ArrowLeft, ArrowRight, X, Layers, PenLine } from "lucide-react";
import { useNavigate, useSearchParams } from "react-router-dom";
import { useAdminAuth } from "../context/AdminAuthContext";
import { listExams, createExam } from "../api/exams";
import { createOrFindPaper, getPaper, submitPaper, listPapers } from "../api/papers";
...
export default function PyqUploadWizard() {
  ...
  if (loadingResume) {
    return (
      <div>
        <PageHeader title="Upload PYQ" subtitle="Loading paper…" />
  ...
  return (
    <div>
      <PageHeader
        title="Upload PYQ"
        subtitle="Step 1: exam  →  Step 2: language  →  Step 3: year  →  Step 4: questions"
      />
...
function PaperDetailsStep({
  exam, language, year, paperCode, onChangeYear, onChangePaperCode,
  ...
  async function handleCheck(e) {
    ...
    setError(null);
    try {
      const res = await listPapers(token, { examId: exam.id, year: Number(year), language });
  ...
  async function handleCreateNewSet(e) {
    ...
    try {
      ...
      if (err.status === 409 && err.data) {
        // Someone else created the exact same code in the moment between our check and this submit --
        // resume that instead of erroring out.
  ...
  function handleChangeYear(setter) {
    return (value) => {
      setter(value);
      setChecked(false);
      setExistingPapers([]);
    ...
    <div className="max-w-md">
      ...
      <form onSubmit={checked ? undefined : handleCheck} className="mt-4 flex flex-col gap-3">
        <FormField label="Year">
          <TextInput required type="number" value={year} onChange={(e) => handleChangeYear(onChangeYear)(e.target.value)} />
        </FormField>
      ...
      {checked && existingPapers.length > 0 && (
        <div className="mt-4">
          <p className="text-xs font-semibold text-ink-600">
            {existingPapers.length} paper{existingPapers.length > 1 ? "s" : ""} already exist{existingPapers.length > 1 ? "" : "s"} for this exam/year/language:
          </p>
  ...
  return (
    <div className="max-w-2xl">
      ...
      {uploadedCount > 0 && (
        <div className="mt-4">
          <Alert type="success">
            {uploadedCount} question{uploadedCount > 1 ? "s" : ""} in this paper so far.
          </Alert>
        </div>
      )}
      ...
      {mode === "bulk" && (
        <div className="mt-4">
          <BulkImportPanel paperId={paper.id} token={token} onImported={onBulkImported} />
        </div>
      )}
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        <Button type="button" variant="secondary" isLoading={finishing} onClick={handleFinish} disabled={uploadedCount === 0}>
          Done — submit paper
        </Button>
      </div>
