...
import { getMyAttempts } from "../../api/mockTests";
import { useAuth } from "../../context/AuthContext";
import { formatDuration } from "../../utils/format";

// Live — wired to GET /api/mocktests/attempts/mine (MockTestsController). Requires
// login (it's per-student history), so guests see a Log In prompt instead of an error.
export default function RecentTests() {
  ...
  useEffect(() => {
    ...
    setStatus("loading");
    getMyAttempts({ page: 1, pageSize: 3 }, { signal: controller.signal })
      .then((res) => {
        setItems(res.items);
        setStatus("success");
      })
    ...
    <section className="px-4 pb-6 sm:px-6 lg:px-0">
      ...
      {status === "success" && items.length === 0 && (
        <p className="rounded-xl2 border border-primary-100 bg-white p-4 text-sm text-ink-400">
          No attempts yet — take your first mock test to see it here.
        </p>
      )}
      ...
      {status === "success" && items.length > 0 && (
        <div className="flex flex-col gap-2.5 sm:gap-3">
          {items.map((test) => (
            <Link
              key={test.attemptId}
              to={`/tests/results/${test.attemptId}`}
            ...
            >
              <div className="flex items-start justify-between gap-2">
                <span className="text-sm font-bold leading-snug text-ink-900">{test.mockTestTitle}</span>
              ...
              <div className="mt-3 flex items-center justify-between">
                <span className="text-lg font-extrabold text-primary-600">
                  {test.score}
                  <span className="text-sm font-medium text-ink-400">/{test.maxPossibleScore}</span>
                </span>
