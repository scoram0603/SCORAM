...
import { MessageSquare, Share2, Flag, Layers } from "lucide-react";
...
// FEED REDESIGN -- one question per card, everything inline (practice options, Discuss,
// Like/Dislike, Share) instead of clicking through to a separate page for each. Reuses the exact
// same LikeButton/CommentThread/SolutionsPanel/ShareQuestionModal components the standalone detail
// page (QuestionBankQuestionDetail.jsx, which still exists as the landing page for shared/direct
// links) uses -- just collapsed behind toggles here since a feed with everything always-expanded
// for every card would be unreadable.
//
...
//
// Practicing IS clicking an option in the grid now -- see InteractiveOptions (green if correct,
// red if wrong, no separate "View Answer" button needed anymore).
export default function QuestionBankFeedCard({ question, onQuestionChange, compact = false }) {
  ...
  return (
    <div className={`rounded-xl2 border border-primary-100 bg-white shadow-card transition-shadow hover:shadow-cardHover ${compact ? "p-3.5" : "p-4 sm:p-5"}`}>
      ...
      {(!compact || expanded) && imgSrc(question.questionImageUrl) && (
        <img src={imgSrc(question.questionImageUrl)} alt="" className="mt-2 max-h-64 rounded-lg border border-primary-100" />
      )}

      {showBody && (
        <div className="mt-4">
          <InteractiveOptions question={question} size={compact ? "sm" : "md"} onInteract={() => compact && setExpanded(true)} />
        </div>
      )}

      {/* Action row -- Discuss/Like/Methods/Share/Report. */}
